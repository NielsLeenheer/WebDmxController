...
{:else if type === 'input'}
    ...
    <div class="preview {className} {sizeClass} {typeClass} {inputTypeClass} {stateClass} {pressedClass}" class:with-3d={needs3DOverflow} style="{euler ? `transform: ${transform3D()};` : ''}">
        ...
        {:else if inputType === 'audio'}
            <!-- Audio input preview: microphone icon -->
            <div class="preview-input audio-preview">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48" class="audio-icon">
                    <path fill="#9fa8da" d="M33,24c0,3.3-2.7,6-6,6h-6c-3.3,0-6-2.7-6-6V10c0-3.3,2.7-6,6-6h6c3.3,0,6,2.7,6,6V24z"/>
